...
// Construct an array of complex numbers
...
function _lifft_setup(x){
	const bits = Math.floor(Math.log2(x.n));
	
	const result = lifft_complex_arr(x.n, x.type);
	const norm = 1/Math.sqrt(result.n);
	for(let i = 0; i < result.n; i++){
		i_rev = _lifft_rev_bits24(i, bits);
		[result.re[i_rev], result.im[i_rev]] = [x.re[i]*norm, x.im[i]*norm];
	}
	return result;
}

// The core Cooley/Tukey algorithm.
function _lifft_process(re, im, n){
	for(let stride = 1; stride < n; stride *= 2){
		const phase = -Math.PI/stride;
		const wm = lifft_complex(Math.cos(phase), Math.sin(phase));
		for(let i = 0; i < n; i += 2*stride){
			...
			for(let j = 0; j < stride; j++){
				...
				[re[idx0], im[idx0]] = [p.re + q.re, p.im + q.im];
				[re[idx1], im[idx1]] = [p.re - q.re, p.im - q.im];
				w = lifft_cmul(w, wm);
			}
		}
	}
